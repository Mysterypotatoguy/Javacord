...
/**
 * An immutable index which maps a every Key to a single Value.
 *
 * @param <K> The type of the key.
 * @param <E> The type of the elements.
 */
public class UniqueIndex<K, E> implements Index<K, E> {
    /**
     ...
     */
    private final Map<K, E> elementsByKey;
     ...
     */
    public UniqueIndex(Function<E, K> keyMapper) {
     ...
     */
    private UniqueIndex(Function<E, K> keyMapper, Map<K, E> elementsByKey, Map<E, K> keyByElement) {
     ...
     */
    public UniqueIndex<K, E> addElement(E element) {
        ...
        Map<K, E> newElementsByKey = elementsByKey.put(key, element);
        Map<E, K> newKeyByElement = keyByElement.put(element, key);
        return new UniqueIndex<>(keyMapper, newElementsByKey, newKeyByElement);
    }
     ...
     */
    public UniqueIndex<K, E> removeElement(E element) {
    ...
    public Set<E> find(K key) {
        return elementsByKey.get(key).isEmpty() ? HashSet.empty() : HashSet.of(elementsByKey.getOrElse(key, null));
    }
